...
def main():
    ...
    word_list,input_list = read_funcdef_from_file('../data/exp2/sample.json')
    # word_list, input_list = read_funcdef_from_file('D:\\EtherscanInfo\\final_without_TSE.json')
    
    filepath = sys.argv[1] # target abi
    outputpath = sys.argv[2] # output file path
    # k = int(sys.argv[3]) #topk
    ...
    for item in target_functions:
        ...
        topk_list = choose_topk_input(sim_matrix, 1)
    ...
    with open(outputpath + '/' + target_abi +".output.txt", "w+") as fp:
